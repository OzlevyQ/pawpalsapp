...
class WebSocketService {
  ...
  initialize(server) {
    console.log('🚀 Initializing WebSocket server on path /notifications-ws...');
    ...
    console.log('✅ WebSocket server created, starting periodic cleanup...');
    ...
    console.log('🎉 WebSocket server fully initialized and listening on /notifications-ws');
    console.log(`📊 Server info: ${this.wss.clients.size} clients connected`);
  }

  // Send notification to specific user
  sendNotificationToUser(userId, notification) {
    console.log(`📤 Attempting to send WebSocket notification to user ${userId}`);
    ...
    if (ws.readyState !== WebSocket.OPEN) {
      console.log(`⚠️ WebSocket connection for user ${userId} is not open. State: ${ws.readyState}`);
    ...
    try {
      ...
      console.log(`📨 Sending WebSocket message to user ${userId}:`, message.substring(0, 200));
      ws.send(message);
