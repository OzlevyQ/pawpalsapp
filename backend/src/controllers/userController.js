...
// Request permission (garden manager or event organizer)
const requestPermission = async (req, res) => {
  try {
    const { requestType } = req.body; // 'garden_manager' or 'event_organizer'
    ...
    if ((requestType === 'garden_manager' && user.role === 'garden_manager') ||
        (requestType === 'event_organizer' && (user.role === 'event_organizer' || user.role === 'admin'))) {
    ...
    for (const admin of admins) {
      try {
        ...
        console.error(`Failed to notify admin ${admin._id}:`, notificationError);
      }
    }

    res.json({ 
      message: `Permission request for ${requestType.replace('_', ' ')} has been sent to administrators`,
      requestType,
      requesterId: req.userId
    });
  } catch (error) {
    console.error('Error requesting permission:', error);
    res.status(500).json({ error: 'Error requesting permission' });
  }
};
