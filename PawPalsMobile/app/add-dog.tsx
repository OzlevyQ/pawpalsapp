...
export default function AddDogScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await dogsApi.addDog(dogPayload);
      ...
      if (response.success) {
        ...
        // Refresh user data to get the new dog
        await reloadDogs();
        
        Alert.alert(
          isRTL ? 'הצלחה!' : 'Success!',
          isRTL ? 'הכלב נוסף בהצלחה' : 'Dog added successfully',
          [
            {
              text: isRTL ? 'אישור' : 'OK',
              onPress: () => router.back(),
      ...
      Alert.alert(
        isRTL ? 'שגיאה' : 'Error',
        isRTL ? 'לא הצלחנו להוסיף את הכלב' : 'Failed to add dog',
        [{ text: isRTL ? 'אישור' : 'OK' }]
      );
        ...
        >
          <View style={{
            ...
            <Text style={{
              ...
              {isRTL ? 'הוספת כלב חדש' : 'Add New Dog'}
            </Text>
            <View style={{ width: 28 }} />
          </View>
        </LinearGradient>
